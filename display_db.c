...
void display_words (main_node_t* temp1, int index)
{
	...
	{
		printf (PURPLE "[%d]\t" RESET, index);
		printf (GREEN "[%-10s]\t" RESET, temp1->word);
		printf (PURPLE "%d" RESET " File(s) :\t", temp1->f_count);
...
void display_files (sub_node_t* temp2)
{
	...
	{
		printf ("File :" GREEN " %s\t" RESET, temp2->f_name);
		printf (PURPLE "%d" RESET " time(s)\t", temp2->w_count);
