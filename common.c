...
void validate_n_store_filenames (file_node_t** files_h, char* filenames[])
{
	...
	while (filenames [i] != NULL)		//The loop shall run to Validate each File name passed in the Command Line.
	{
		...
		if (ret == SUCCESS)		//If the File name is valid, proceed to check if it is repeated or not.
		{
			...
			if (ret == FAILURE)	//If the File name is not present in the File List; proceed to add it into the File List.
			{
				...
				{
					printf (WHITE "INFO: The File %s is added to the List Successfully.\n" RESET, filenames [i]);
				}
			}
		}
	...
	{
		printf (RED "ERROR: There is no Valid File passed as Input.\n" YELLOW "INFO: Please enter names of Valid Files only.\n" RESET);
...
int IsFileValid (char* filename)
{
	...
	{
		printf (RED "ERROR: The File %s is not present.\n" YELLOW "INFO: So, this File cannot be added into the Database.\n" RESET, filename);
		return NOT_PRESENT;
	}
	else			//The File is Opened successfully.
	{
		...
		{
			printf (RED "ERROR: The File %s is an Empty File.\n" YELLOW "INFO: So, this File cannot be added into the Database.\n" RESET, filename);
			return FILE_EMPTY;
		}
		else
		{
...
int store_filenames_to_list (char* f_name, file_node_t** head)
{
	...
	{
		printf (RED "ERROR: Unable to add the File %s into the File List.\n" RESET, f_name);
		return FAILURE;
	}
